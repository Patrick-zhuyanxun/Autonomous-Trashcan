...
void LslidarX10Driver::pubScanThread()
{
  ...
  while (ros::ok())
  {
    ...
    if(lidar_name == "N10") 
    {
        if( (msg.intensities[point_idx] <=5 && msg.intensities[point_idx] >0) || (msg.intensities[point_idx] <=255 && msg.intensities[point_idx] >250) )
        {
            msg.ranges[point_idx] = std::numeric_limits<float>::infinity();
            msg.intensities[point_idx] =0;
        }
    }
    if(lidar_name == "N10" || lidar_name == "M10" || lidar_name == "M10_P" || lidar_name == "M10_PLUS")
    {
        if(msg.intensities[point_idx] == 255)
        {
            msg.ranges[point_idx] = std::numeric_limits<float>::infinity();
            msg.intensities[point_idx] = 0;
        }
